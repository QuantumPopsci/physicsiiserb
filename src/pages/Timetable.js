...
const Timetable = () => {
  // A map for quick course lookup by code. This handles codes like "PHY 425/625".
  ...
  // Pre-process the schedule to handle multi-hour spans and clashing events.
            ...
            {schedule.timeSlots.map(time => (
              <tr key={time} className="h-28">
                ...
                {schedule.days.map(day => {
                  ...
                  if (cellContent === 'occupied') return null; // This cell is covered by a multi-hour class
                  ...
                  return <td key={`${day}-${time}`} className="border-t border-border-color"></td>; // Empty cell
                })}
              </tr>
            ))}
          </tbody>
